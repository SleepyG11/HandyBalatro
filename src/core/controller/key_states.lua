...
local function pre_press_key(input_type, raw_key)
	local input_ctx = Handy.controller.input.update_context(input_type, raw_key, false, 0)
...
local function post_press_key()
	local input_ctx = Handy.controller.input.get_context()
	...
	Handy.controller.input.update_context()

	return input_ctx
end

local function pre_release_key(input_type, raw_key)
	local input_ctx = Handy.controller.input.update_context(input_type, raw_key, true)
...
local function is_key_trigger(key, released)
	if not key then
		return false
	elseif not Handy.controller.keys.is_holdable_key(key) then
		return not released
	else
...
Handy.controller.key_states = key_states

Handy.controller.is_key_press = is_key_press
Handy.controller.is_key_release = is_key_release
Handy.controller.is_key_trigger = is_key_trigger

Handy.controller.is_key_hold = is_key_hold
Handy.controller.is_keys_hold = is_keys_hold
Handy.controller.is_keys_count_match = is_keys_count_match
